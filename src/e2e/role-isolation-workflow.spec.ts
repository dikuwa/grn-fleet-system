...
test.describe.serial('Approved multi-role workflow and isolation', () => {
  ...
  test('every operational account logs in and receives role-scoped API access', async ({
    ...
    await requesterUi.page.screenshot({
      path: 'docs/screenshots/requester-mobile.png',
      fullPage: true,
    });
  ...
  test('regional request executes through separate users and persists evidence', async ({
    browser,
  }) => {
    const requester = await login(accounts.requester);
    ...
    const requestResponse = await requester.post('/api/transport-requests', {
      ...
      data: {
        purpose: 'Multi-role E2E regional field visit',
        scope: 'regional',
        activities: [
          {
            title: 'Field visit',
    ...
    const duplicate = await requester.post('/api/transport-requests', {
      headers: { 'idempotency-key': clientSubmissionId },
      data: { purpose: 'duplicate', scope: 'regional' },
    });
    ...
    const availableVehicles = fleetRows.filter(
    ...
    const departure = await inspector.post('/api/inspections', {
      data: {
        ...
        tripId,
        type: 'departure',
        odometerReading: availableVehicles[0].currentOdometer,
        fuelLevel: 'full',
    ...
    const issue = await transport.post(`/api/trips/${tripId}/issue`, {
      data: {
        keysIssued: true,
        fuelCardIssued: true,
        issueOdometer: availableVehicles[0].currentOdometer,
      },
    });
    expect(issue.status(), await issue.text()).toBe(200);
    const startTrip = await driver.post(`/api/trips/${tripId}/start`, {
      data: {
        beginningOdometer: availableVehicles[0].currentOdometer,
    ...
    const fuel = await driver.post('/api/fuel', {
      data: {
        tripId,
        vehicleId,
        clientSyncId: fuelSyncId,
        fuelType: 'diesel',
        litres: 35,
        amount: 720,
    ...
    const tripLog = await driver.post('/api/trip-logs', {
      data: {
        tripId,
        clientSyncId: logSyncId,
        logDate: new Date().toISOString().slice(0, 10),
        odometerOut: availableVehicles[0].currentOdometer,
        odometerIn: availableVehicles[0].currentOdometer + 60,
        origin: 'Rundu',
        destination: 'Divundu',
        distanceKm: 60,
      },
    ...
    const duplicateLog = await driver.post('/api/trip-logs', {
      data: { tripId, clientSyncId: logSyncId, logDate: new Date().toISOString().slice(0, 10) },
    });
    expect((await duplicateLog.json()).idempotent).toBe(true);

    const returned = await driver.post(`/api/trips/${tripId}/return`, {
      data: {
        endingOdometer: availableVehicles[0].currentOdometer + 60,
        fuelLevel: 'half',
    ...
    const verifyFuel = await transport.patch('/api/fuel', {
      data: { transactionId: fuelTransactionId, action: 'verify' },
    });
    expect(verifyFuel.status(), await verifyFuel.text()).toBe(200);
    const close = await transport.post(`/api/trips/${tripId}/close`, {
      data: {
        decision: 'closed',
        reviewNotes: 'Closed operationally; blocking defect retained for maintenance.',
      },
    });
    ...
    const requestAudits = await db
      .select()
      .from(auditEvents)
      .where(eq(auditEvents.tenantId, savedRequest.tenantId));
    const workflowNotifications = await db
      .select()
      .from(notifications)
      .where(eq(notifications.entityId, workflowId));
    ...
    expect(
      requestAudits.some((event) => event.entityId === tripId && event.eventType === 'trip_closed'),
    ).toBe(true);
    ...
    await maintenanceUi.page.screenshot({
      path: 'docs/screenshots/maintenance-escalation.png',
      fullPage: true,
    });
  ...
  test('rejection, resubmission, cancellation, and national routing use the assigned people', async () => {
    ...
    await approve(
      ...
      'rejected',
      'Clarify the operational justification.',
    );
    const resubmission = await requester.post(`/api/requests/${rejected.id}/resubmit`, {
      data: { reason: 'Justification and passenger details corrected.' },
    });
    ...
    const cancellation = await requester.patch(`/api/requests/${cancellable.id}/cancel`, {
      data: { reason: 'Journey no longer required.' },
    });
    ...
    const [cancelledRequest] = await db
      .select()
      .from(transportRequests)
      .where(eq(transportRequests.id, cancellable.id))
      .limit(1);
    const [nationalRequest] = await db
      .select()
      .from(transportRequests)
      .where(eq(transportRequests.id, national.id))
      .limit(1);
    const [cancelAudit] = await db
      ...
      .where(
        and(
          eq(auditEvents.entityId, cancellable.id),
          eq(auditEvents.eventType, 'request_cancelled'),
        ),
      )
      .limit(1);
    ...
    await Promise.all(
      [requester, supervisor, transport, nationalRelease, nationalAuthoriser, driver].map((api) =>
        api.dispose(),
      ),
    );
  });
