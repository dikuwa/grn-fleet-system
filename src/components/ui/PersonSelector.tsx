...
interface PersonSelectorProps {
  ...
  // Optional: pre-selected values (ids)
  defaultValue?: string[];
...
export function PersonSelector({
  ...
  // Convert selectedIds to objects for display (we don't fetch the full object unless needed)
  ...
  const fetchOptions = useCallback(
    async (q: string, pageNum: number) => {
      ...
      try {
        ...
        // Note: we are using the people-search endpoint without specifying kind (defaults to employee)
        // If we want to restrict to drivers, we would add &kind=driver
        ...
        console.error('PersonSelector fetch error:', err);
  ...
  const onCallBack = (selected: PersonOption[]) => {
    ...
    if (multiple) {
      return selectedOptions.map((opt) => (
        <div key={opt.id} className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-primary/10 text-primary">
          {opt.fullName} ({opt.employeeNumber})
          <button
            type="button"
            ...
            aria-label={`Remove ${opt.fullName}`}
          >
            &times;
          </button>
        </div>
      ...
      return (
        <div className="flex items-center space-x-2">
          <div className="text-xs font-medium">{opt.fullName}</div>
          <div className="text-xs text-muted-foreground">{opt.employeeNumber}</div>
        </div>
      );
    }
  };
          ...
          {options.map((opt) => (
            ...
            >
              <div className="flex-1 min-w-0">
                <div className="font-medium">{opt.firstName} {opt.lastName}</div>
