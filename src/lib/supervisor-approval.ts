...
const SUPERVISOR_RESULTS = ['approved', 'rejected', 'returned'] as const;
type SupervisorResult = (typeof SUPERVISOR_RESULTS)[number];

/**
 * Commit a supervisor decision as one PostgreSQL statement.
 *
 * The legacy workflow engine persisted workflow_actions first, then advanced
 * workflow_instances and transport_requests separately. A later failure could
 * leave an action that blocked retry while the request still sat on the old
 * stage. This helper makes the Supervisor step race-safe without duplicating
 * the engine's conflict-reassignment behaviour.
 */
export async function processSupervisorDecisionAtomic(input: {
  instanceId: string;
  ...
  if (!SUPERVISOR_RESULTS.includes(result as SupervisorResult)) {
    return {
      ok: false,
      error: NextResponse.json({ error: 'Invalid supervisor decision.' }, { status: 400 }),
    };
  }
  ...
  if (!status?.currentStep) {
    return {
      ok: false,
      error: NextResponse.json({ error: 'Workflow step not found.' }, { status: 404 }),
    };
  }
  ...
  if (currentStep.actionType !== 'supervisor_approve') {
    return {
      ok: false,
      error: NextResponse.json({ error: 'This workflow is no longer awaiting supervisor approval.' }, { status: 409 }),
    };
  }
  ...
  if (currentStep.assignedUserId && currentStep.assignedUserId !== session.user.id) {
    return {
      ok: false,
      error: forbiddenResponse('This supervisor approval is assigned to another responsible user.'),
    };
  }
  ...
  if (!requestRecord) {
    return {
      ok: false,
      error: NextResponse.json({ error: 'Workflow request not found.' }, { status: 404 }),
    };
  }
  ...
  const workflowStatus = result === 'approved' ? (nextStep ? 'active' : 'completed') : 'cancelled';
  const nextOrder = result === 'approved' && nextStep ? nextStep.stepOrder : currentStep.stepOrder;
  ...
  const signatureRef =
    signatureProfile?.confirmedAt && result === 'approved'
  ...
  try {
    ...
    if (committed !== 1) {
      return {
        ok: false,
        error: NextResponse.json(
          { error: 'This approval changed while you were deciding it. Refresh before trying again.' },
          { status: 409 },
        ),
      };
    }
  } catch (error) {
    console.error('[supervisor-approval] Atomic decision failed:', error);
    ...
    if (latest?.instance.status !== 'active' || latest?.instance.currentStepOrder !== currentStep.stepOrder) {
      return {
        ok: false,
        error: NextResponse.json(
          { error: 'This approval has already moved to another state. Refresh to see the latest decision.' },
          { status: 409 },
        ),
      };
    }
  ...
  }).catch((error) => console.warn('[supervisor-approval] Could not resolve old notification:', error));
  ...
  }).catch((error) => console.warn('[supervisor-approval] Activity update failed:', error));
  ...
  if (requestRecord.requesterUserId) {
    await createScopedNotifications({
      ...
      body:
        result === 'approved'
          ? 'Your transport request passed supervisor review and moved to the next stage.'
          : comment?.trim() || `Your request was ${result}.`,
      entityType: 'workflow_instance',
      entityId: instanceId,
    ...
    }).catch((error) => console.warn('[supervisor-approval] Requester notification failed:', error));
  }
  ...
  if (result === 'approved' && nextStep) {
    ...
    if (nextRecipients.length > 0) {
      await createScopedNotifications({
        ...
        priority: 'high',
      }).catch((error) => console.warn('[supervisor-approval] Next-step notification failed:', error));
    }
    ...
    message:
      result === 'approved'
        ? nextStep
          ...
          : 'Workflow completed. Request approved.'
