...
const source = readFileSync(
  resolve(process.cwd(), 'src/app/api/departments/[id]/route.ts'),
  'utf8',
);
...
describe('department dynamic UUID guards', () => {
  it('guards PATCH route ids before forwarding into the UUID-backed shared route', () => {
    ...
    const forwardIndex = patchSource.indexOf('return patchDepartment(');
    ...
    expect(patchSource).toContain("{ error: 'Organisation unit not found.' }, { status: 404 }");
  });
  ...
  it('keeps DELETE authorization ahead of the UUID guard and database access behind it', () => {
    ...
    const lookupIndex = deleteSource.indexOf('.from(departments)');
    ...
    expect(deleteSource).toContain("{ error: 'Organisation unit not found.' }, { status: 404 }");
  });
});
