...
// to every Transport Administrator summarising all driver licences that are
// already expired or expire within the next 60 days — idempotent per tenant
// per day via a day-epoch eventVersion key.
//
// The digest logic lives in `src/lib/inngest/expiry-digest.ts` so it can be
// exercised directly by integration tests; the cron is a thin call site.
// ---------------------------------------------------------------------------
  ...
  ? inngest.createFunction(
      ...
      async ({ step }) => {
        return step.run('Send daily driver licence digest to transport admins', async () => {
          ...
          return runDriverLicenceExpiryDigest();
        });
      },
    )
  : null;
