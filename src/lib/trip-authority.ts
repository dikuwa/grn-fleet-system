 ...
 * Existing request, allocation, vehicle, route, passenger and driver data is copied,
 * not requested from the user a second time.
 */
export async function provisionTripAuthority(input: {
  ...
  const [existing] = await db
    ...
    .where(eq(tripAuthorities.tripId, input.tripId))
    .limit(1);
  if (existing) return { authority: existing, verificationToken: null };
  ...
  if (passengerRows.length) {
    await db.insert(tripAuthorityPassengers).values(passengerRows.map((passenger) => ({
  ...
  await db.insert(tripAuthorisedDrivers).values({
    authorityId: authority.id,
    ...
    licenceExpiry: new Date(`${driver.licenceExpiry}T23:59:59Z`),
