...
async function fetchOfficesData() {
  const dbo = getDb();
  const allOffices = await dbo.select().from(offices).where(eq(offices.isActive, true)).orderBy(asc(offices.name));
  const allDepartments = await dbo.select().from(departments).where(eq(departments.isActive, true)).orderBy(asc(departments.name));
  ...
  const officeCounts = await dbo
    ...
    .from(employees)
    .where(and(eq(employees.employmentStatus, 'active'), isNotNull(employees.officeId)))
  ...
  const deptCounts = await dbo
    ...
    .from(employees)
    .where(and(eq(employees.employmentStatus, 'active'), isNotNull(employees.departmentId)))
    ...
    return (
      <div className="space-y-6">
        ...
        <EmptyState
          ...
          description="Set the DATABASE_URL environment variable and run migrations to view the office hierarchy."
        />
      </div>
    );
  }
    ...
    return (
      <div className="space-y-6">
        ...
        <EmptyState
          ...
          description="The database query failed. Please ensure migrations have been run."
        />
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        <Card className="lg:col-span-1">
          <CardHeader>
            <CardTitle>Departments & Directorates</CardTitle>
          </CardHeader>
          <CardContent>
                  ...
                  <div key={dept.id} className="flex items-center justify-between py-3 first:pt-0 last:pb-0">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        {dept.code && (
                          <p className="text-xs text-ink-500">Code: {dept.code}</p>
                        )}
                      </div>
                    </div>
