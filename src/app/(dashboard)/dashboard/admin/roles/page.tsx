...
export default function AdminRolesPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      <PageHeader title="Roles & Permissions" description={`${roles.length} tenant role${roles.length === 1 ? '' : 's'} configured. Permission sets are editable; built-in role names remain fixed for workspace routing.`}>
          ...
          {visibleRoles.map((role) => (
            <article key={role.id} className="rounded-[10px] border border-border bg-surface p-4">
              <div className="flex items-start justify-between gap-3">
                ...
                <Button variant="ghost" size="compact" onClick={() => openEdit(role)} aria-label={`Edit ${role.name}`}><Pencil className="h-4 w-4" /></Button>
              </div>
            ...
            <tbody className="divide-y divide-border">
              {visibleRoles.map((role) => (
                <tr key={role.id}>
                  ...
                  <td className="px-4 py-3 text-right"><Button variant="secondary" size="compact" onClick={() => openEdit(role)}><Pencil className="h-3.5 w-3.5" /> Edit</Button></td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      <Dialog open={Boolean(editRole)} onOpenChange={(open) => !open && setEditRole(null)}>
        <DialogContent className="max-h-[92dvh] overflow-y-auto sm:max-w-3xl">
          <DialogHeader><DialogTitle>Edit role</DialogTitle><DialogDescription>Changes apply to every active member who holds this role. Built-in names remain fixed because they select application workspaces.</DialogDescription></DialogHeader>
          <div className="space-y-4">
            <div className="grid gap-4 sm:grid-cols-2"><div className="space-y-1.5"><Label required>Role name</Label><Input value={editName} onChange={(event) => setEditName(event.target.value)} disabled={editRole?.isSystem} />{editRole?.isSystem && <p className="text-xs text-ink-500">Built-in names are fixed; permissions and description are editable.</p>}</div><div className="space-y-1.5"><Label>Description</Label><Input value={editDescription} onChange={(event) => setEditDescription(event.target.value)} /></div></div>
