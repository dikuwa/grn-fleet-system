...
export default function PlatformDemoRequestsPage() {
  ...
  // openDetails only maps local state; keeping it out prevents unnecessary refetch loops.
  ...
  const updateRequest = async (patch: Record<string, unknown>, successMessage: string) => {
    ...
    try {
      ...
      const updated = { ...selected, ...json.data } as DemoRequest;
      setSelected(updated);
  ...
  const saveSchedule = async () => {
    ...
    await updateRequest({
      status: 'scheduled',
      scheduledDemoAt: new Date(scheduleAt).toISOString(),
      scheduledDemoLink: scheduleLink,
      contactNotes,
  ...
  const revokeSandbox = async () => {
    if (!selected?.sandbox) return;
    setSaving(true);
    try {
      const res = await fetch('/api/platform/demo-requests/sandbox', {
        method: 'PATCH', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ demoRequestId: selected.id, action: 'revoke' }),
      });
      const json = await res.json();
      if (!res.ok) throw new Error(json.error || 'Sandbox could not be revoked');
      toast({ title: 'Sandbox revoked', variant: 'success' });
      await loadRequests();
      setSelected((current) => current ? { ...current, sandbox: { ...current.sandbox!, status: 'revoked' } } : current);
    } catch (err) {
      toast({ title: 'Could not revoke sandbox', description: err instanceof Error ? err.message : 'Update failed', variant: 'error' });
  ...
  const canSandbox = selected && ['qualified', 'scheduled', 'completed'].includes(selected.status) && !selected.sandbox;
  ...
  return (
    <div className="space-y-6">
      ...
      {stats && (
        <section aria-label="Demo request summary" className="grid overflow-hidden rounded-[10px] border border-border bg-border grid-cols-2 sm:grid-cols-3 lg:grid-cols-6">
          {STAT_ITEMS.map(([key, label]) => <div key={key} className="bg-surface px-4 py-4"><p className="text-2xl font-semibold tabular-nums text-ink-950">{stats[key] ?? 0}</p><p className="mt-0.5 text-xs text-ink-500">{label}</p></div>)}
        </section>
      )}
        ...
        : (
          <div className="overflow-hidden rounded-[10px] border border-border bg-surface">
            {requests.map((request) => {
              ...
              return (
                <button key={request.id} type="button" onClick={() => openDetails(request)} className="focus-ring block w-full border-b border-border px-4 py-4 text-left transition-colors last:border-b-0 hover:bg-muted/40 sm:px-5 motion-reduce:transition-none">
                  <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                    <div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><Building2 className="h-4 w-4 text-ink-400" /><p className="truncate text-sm font-semibold text-ink-950">{request.company}</p><Badge variant={config.variant} size="sm">{config.label}</Badge>{request.sandbox && <Badge variant={request.sandbox.status === 'active' ? 'success' : 'default'} size="sm">sandbox {request.sandbox.status}</Badge>}</div><p className="mt-1 truncate text-xs text-ink-500">{request.name} · {request.jobTitle} · {request.email}</p></div>
                    <div className="flex shrink-0 items-center gap-3 text-xs text-ink-500">{request.vehicleCount ? <span className="flex items-center gap-1"><Car className="h-3.5 w-3.5" />{request.vehicleCount} vehicles</span> : null}<span>{displayDate(request.createdAt)}</span><ArrowRight className="h-4 w-4 text-ink-300" /></div>
                  </div>
                </button>
              );
            })}
      ...
      <Dialog open={Boolean(selected)} onOpenChange={(open) => { if (!open) setSelected(null); }}>
        <DialogContent className="max-h-[92dvh] overflow-y-auto sm:max-w-3xl">
          {selected && selectedConfig && <>
            <DialogHeader><div className="flex flex-wrap items-center gap-2"><DialogTitle>{selected.company}</DialogTitle><Badge variant={selectedConfig.variant} size="sm">{selectedConfig.label}</Badge></div><DialogDescription>Demo request from {selected.name} · received {displayDate(selected.createdAt)}</DialogDescription></DialogHeader>

            <div className="space-y-5">
              <section className="grid gap-3 rounded-[8px] border border-border bg-muted/25 p-4 sm:grid-cols-2 lg:grid-cols-3">
                <div><p className="text-xs font-medium text-ink-400">Contact</p><p className="mt-1 text-sm font-semibold text-ink-950">{selected.name}</p><p className="text-xs text-ink-500">{selected.jobTitle}</p><a href={`mailto:${selected.email}`} className="mt-1 flex items-center gap-1 text-xs text-brand-700 hover:underline"><Mail className="h-3.5 w-3.5" />{selected.email}</a>{selected.phone && <a href={`tel:${selected.phone}`} className="mt-1 flex items-center gap-1 text-xs text-brand-700 hover:underline"><Phone className="h-3.5 w-3.5" />{selected.phone}</a>}</div>
                <div><p className="text-xs font-medium text-ink-400">Organisation</p><p className="mt-1 text-sm text-ink-800">{selected.industry || 'Type not specified'}</p><p className="mt-1 flex items-center gap-1 text-xs text-ink-500"><Users className="h-3.5 w-3.5" />{selected.userCount ?? '—'} expected users</p><p className="mt-1 flex items-center gap-1 text-xs text-ink-500"><Car className="h-3.5 w-3.5" />{selected.vehicleCount ?? '—'} vehicles</p></div>
                <div><p className="text-xs font-medium text-ink-400">Preference</p><p className="mt-1 text-sm text-ink-800">{displayDate(selected.preferredDate)}</p><p className="text-xs text-ink-500">{selected.preferredTime || 'Flexible time'} · {selected.contactMethod}</p><p className="mt-1 text-xs text-ink-500">Source: {selected.source || 'website'}</p></div>
              </section>
              ...
              {selected.sandbox ? <section className="rounded-[8px] border border-border p-4"><div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div><div className="flex items-center gap-2"><ShieldCheck className="h-4 w-4 text-brand-700" /><h3 className="text-sm font-semibold text-ink-950">Walkthrough sandbox</h3><Badge variant={selected.sandbox.status === 'active' ? 'success' : 'default'} size="sm">{selected.sandbox.status}</Badge></div><p className="mt-1 text-xs text-ink-500">Temporary isolated tenant · expires {displayDate(selected.sandbox.expiresAt, true)}</p><p className="mt-1 text-xs text-ink-500">Login email: {selected.sandbox.adminEmail}</p></div>{selected.sandbox.status === 'active' && <Button variant="destructive" size="sm" onClick={() => void revokeSandbox()} loading={saving}>Revoke sandbox</Button>}</div></section>
