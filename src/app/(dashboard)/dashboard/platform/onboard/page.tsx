...
interface OnboardingForm {
  ...
  timezone: string;
  locale: string;
  transportProcess: 'request_led' | 'programme_led' | 'mixed';
  budgetControl: 'conditional' | 'always' | 'none';
...
const DEFAULT_FORM: OnboardingForm = {
  ...
  transportProcess: 'mixed', budgetControl: 'conditional', acceptsExternalSponsoredRequests: false,
...
export default function OnboardTenantPage() {
    ...
    mutationFn: async () => {
      ...
      const res = await fetch('/api/platform/onboard', {
        ...
        body: JSON.stringify({
          ...
          transportProcess: { model: form.transportProcess, budgetControl: form.budgetControl, acceptsExternalSponsoredRequests: form.acceptsExternalSponsoredRequests },
  ...
  const renderStep = () => {
    switch (form.currentStep) {
      case 0:
        return <div className="space-y-4"><div className="space-y-1.5"><Label required>Organisation name</Label><Input value={form.orgName} onChange={(event) => updateOrganisationName(event.target.value)} placeholder="Organisation name" /></div><div className="grid gap-4 sm:grid-cols-2"><div className="space-y-1.5"><Label required>Code</Label><Input value={form.orgCode} onChange={(event) => updateField('orgCode', event.target.value.toUpperCase())} className="font-mono" /></div><div className="space-y-1.5"><Label required>Slug</Label><Input value={form.orgSlug} onChange={(event) => updateField('orgSlug', makeSlug(event.target.value))} className="font-mono" /></div><div className="space-y-1.5"><Label>Organisation type</Label><StyledSelect value={form.orgType} onChange={(event) => updateField('orgType', event.target.value)}>{ORG_TYPES.map((item) => <option key={item.value} value={item.value}>{item.label}</option>)}</StyledSelect></div><div className="space-y-1.5"><Label>Timezone</Label><StyledSelect value={form.timezone} onChange={(event) => updateField('timezone', event.target.value)}><option value="Africa/Windhoek">Africa/Windhoek (UTC+2)</option></StyledSelect></div></div><div className="rounded-[8px] border border-border p-4"><h3 className="text-sm font-semibold text-ink-950">Transport process</h3><p className="mt-1 text-xs text-ink-500">These answers generate editable workflow recommendations. They do not activate an organisation mode or publish a route.</p><div className="mt-4 grid gap-4 sm:grid-cols-2"><div className="space-y-1.5"><Label>Primary intake model</Label><StyledSelect value={form.transportProcess} onChange={(event) => updateField('transportProcess', event.target.value as OnboardingForm['transportProcess'])}><option value="request_led">Ad hoc request-led</option><option value="programme_led">Programme-led</option><option value="mixed">Mixed programme and ad hoc</option></StyledSelect></div><div className="space-y-1.5"><Label>Budget review</Label><StyledSelect value={form.budgetControl} onChange={(event) => updateField('budgetControl', event.target.value as OnboardingForm['budgetControl'])}><option value="conditional">When financial impact exists</option><option value="always">Every request</option><option value="none">Not a transport workflow gate</option></StyledSelect></div></div><label className="mt-4 flex items-start gap-2 text-sm text-ink-700"><input type="checkbox" className="mt-0.5" checked={form.acceptsExternalSponsoredRequests} onChange={(event) => updateField('acceptsExternalSponsoredRequests', event.target.checked)} /><span>External or sponsored transport requests are accepted</span></label></div></div>;
      case 1:
