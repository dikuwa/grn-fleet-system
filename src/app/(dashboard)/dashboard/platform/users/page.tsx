...
export default function PlatformUsersPage() {
  ...
  const createUser = async () => {
    ...
    try {
      ...
      toast({
        ...
        description: `${json.data.name} can now access the platform workspace.`,
        variant: 'success',
      });
      await load();
    } catch (err) {
      toast({
        title: 'Could not create platform user',
  ...
  const saveUser = async () => {
    ...
    try {
      ...
      toast({
        title: 'Update failed',
        description: err instanceof Error ? err.message : 'Could not update platform user',
        variant: 'error',
      });
    } finally {
      setSaving(false);
  ...
  const removeUser = (platformUser: PlatformUser) => {
    confirm({
      ...
      description:
        'The account is retained for audit safety, but its platform membership and role access are removed. The final Platform Super Administrator can never be removed.',
      confirmLabel: 'Remove access',
      variant: 'destructive',
      onConfirm: async () => {
        const res = await fetch(
          `/api/platform/users?userId=${encodeURIComponent(platformUser.id)}`,
          { method: 'DELETE' },
        );
        const json = await res.json();
  ...
  return (
    <div className="space-y-6">
      <Breadcrumbs
        items={[{ label: 'Platform', href: '/dashboard/platform' }, { label: 'Platform Users' }]}
      />
      <PageHeader
        title="Platform Users"
        description="Delegate platform operations without sharing the primary administrator account."
      >
      ...
      <section className="border-brand-200 bg-brand-50/50 dark:border-brand-900/70 dark:bg-brand-950/20 rounded-[10px] border p-4">
        <div className="flex items-start gap-3">
          <Shield className="text-brand-700 dark:text-brand-300 mt-0.5 h-5 w-5 shrink-0" />
          <div>
            <p className="text-ink-950 text-sm font-semibold">Protected administrator continuity</p>
      ...
      {loading ? (
        <div className="text-ink-500 flex min-h-48 items-center justify-center text-sm">
          Loading platform users…
        </div>
      ) : error ? (
        <EmptyState
          icon={<UserCog className="h-6 w-6" />}
          title="Could not load platform users"
          description={error}
        ...
        <EmptyState
          ...
          description="Add a trusted platform operator to help manage GovFleet."
          action={{ label: 'Add platform user', onClick: () => setCreateOpen(true) }}
        />
      ) : (
        <div className="border-border bg-surface overflow-hidden rounded-[10px] border">
          {users.map((platformUser) => (
            <article
              key={platformUser.id}
              className="border-border grid gap-4 border-b px-4 py-4 last:border-b-0 sm:px-5 lg:grid-cols-[minmax(0,1fr)_260px_auto] lg:items-center"
            >
              <div className="min-w-0">
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  {platformUser.isCurrentUser && (
                    <Badge variant="info" size="sm">
                      You
                    </Badge>
                  )}
                  <Badge
                    variant={platformUser.status === 'active' ? 'success' : 'default'}
                    size="sm"
                  >
                    {platformUser.status.replace(/_/g, ' ')}
                  </Badge>
                </div>
                <p className="text-ink-500 mt-1 truncate text-xs">
                  {platformUser.email}
              ...
              <div className="flex flex-wrap gap-2 lg:justify-end">
                <Button
                  variant="secondary"
                  size="sm"
                  onClick={() => setEditing({ ...platformUser })}
                >
                ...
                {!platformUser.isCurrentUser && (
                  <Button
                    ...
                    onClick={() => removeUser(platformUser)}
                  >
        ...
        <DialogContent className="sm:max-w-md">
          ...
          <div className="space-y-4">
            <div className="space-y-1.5">
              <Label htmlFor="platform-user-name" required>
                Name
              </Label>
              <Input
            ...
            <div className="space-y-1.5">
              <Label htmlFor="platform-user-email" required>
                Email
              </Label>
              <Input
            ...
            <div className="space-y-1.5">
              <Label htmlFor="platform-user-role" required>
                Role
              </Label>
              <StyledSelect
          ...
          <DialogFooter>
            ...
            <Button onClick={() => void createUser()} loading={saving}>
              <UserRoundCheck className="h-4 w-4" /> Create user
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
            ...
            <>
              ...
              <div className="space-y-4">
                <div className="space-y-1.5">
                  <Label>Role</Label>
                  <StyledSelect
                    value={editing.roleName || SystemRoles.PLATFORM_SUPPORT}
                    onChange={(event) =>
                      setEditing((current) =>
                        current ? { ...current, roleName: event.target.value } : current,
                      )
                    }
                  >
                    {ROLE_OPTIONS.map((role) => (
                ...
                <div className="space-y-1.5">
                  ...
                  {editing.isCurrentUser && (
                    <p className="text-ink-500 text-xs">
                      Your own active platform access cannot be disabled here.
                    </p>
                  )}
                </div>
              </div>
      ...
      <Dialog open={Boolean(created)} onOpenChange={(open) => !open && setCreated(null)}>
        <DialogContent className="sm:max-w-lg">
          {created && (
            <>
              <DialogHeader>
                <DialogTitle>Platform user created</DialogTitle>
                <DialogDescription>
                  The temporary password is shown once. Share it through an appropriate private
                  channel.
                </DialogDescription>
              </DialogHeader>
              <div className="border-border bg-muted/30 space-y-2 rounded-[8px] border p-4 font-mono text-sm">
                <p>
                  <span className="text-ink-500">Name:</span> {created.name}
                </p>
                <p>
                  <span className="text-ink-500">Email:</span> {created.email}
                </p>
                <p>
                  <span className="text-ink-500">Username:</span> {created.username}
                </p>
                <p>
                  <span className="text-ink-500">Role:</span> {created.roleName}
                </p>
                <p>
