...
const reportTypes: {
...
}[] = [
  {
    ...
    description: 'Fuel usage, costs, and reimbursement tracking',
  },
  {
    value: 'fleet',
    label: 'Fleet Utilisation',
    icon: <Truck className="h-4 w-4" />,
    description: 'Vehicle availability, usage rates, and status distribution',
  },
  {
    value: 'trips',
    label: 'Trip Summary',
    icon: <Gauge className="h-4 w-4" />,
    description: 'Trip volumes, distances, durations, and completions',
  },
  {
    value: 'maintenance',
    label: 'Maintenance',
  ...
  },
  {
    ...
    description: 'Request volumes, processing times, and status breakdown',
  },
  {
    value: 'approvals',
    label: 'Approvals',
    icon: <ClipboardList className="h-4 w-4" />,
    description: 'Approval turnaround times and workflow metrics',
  },
  {
    value: 'enhanced',
    ...
    description:
      'Approval detail, vehicle utilisation, fuel efficiency, late returns, rejection metrics',
  },
];
...
function BarChart({
  ...
  if (data.length === 0) {
    return (
      <p className="text-ink-400 py-8 text-center text-xs">No data available for this period.</p>
    );
  }
    ...
    <div className="flex items-end gap-3" style={{ height }}>
      {data.map((d, i) => (
        <div key={i} className="flex h-full flex-1 flex-col items-center justify-end gap-1">
          <div className="flex w-full flex-col items-center gap-0.5" style={{ minHeight: 0 }}>
            {bars.map((b) => {
          ...
          <span className="text-ink-500 mt-1 text-center text-[10px]">
            {d.label || d.month || ''}
          </span>
        </div>
      ))}
  ...
  return (
    <div className="space-y-6">
      <div className="no-print">
        <Breadcrumbs
          items={[{ label: 'Dashboard', href: '/dashboard' }, { label: 'Reports & Analytics' }]}
        />
        <PageHeader
          ...
          <div className="flex flex-wrap gap-3">
            {reportTypes.map((r) => (
              <button
                ...
                aria-pressed={selectedReport === r.value}
            ...
            <Link
              ...
              className="bg-muted text-ink-700 hover:bg-border inline-flex items-center gap-2 rounded-[8px] px-3 py-2 text-sm font-medium transition-colors"
            >
              <Shield className="h-4 w-4" />
              Licence Expiry
            </Link>
          </div>
          {activeReport && <p className="text-ink-500 mt-2 text-xs">{activeReport.description}</p>}
        </CardContent>
      </Card>
                  ...
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="border-border border-b">
                        <th className="text-ink-500 px-2 py-2 text-left text-xs font-medium tracking-wider uppercase">
                          Service Type
                        </th>
                        <th className="text-ink-500 px-2 py-2 text-right text-xs font-medium tracking-wider uppercase">
                          Events
                        </th>
                        <th className="text-ink-500 px-2 py-2 text-right text-xs font-medium tracking-wider uppercase">
                          Total Cost
                    ...
                    <tbody>
                      {(() => {
                        ...
                        if (!stats || stats.length === 0) {
                          return (
                            <tr>
                              <td colSpan={3} className="text-ink-400 py-8 text-center text-xs">
...
function ReportFuel({ data }: { data: Record<string, unknown> | null }) {
  const summary = data?.summary as
    | {
        ...
        avgCostPerLitre: string;
      }
    | undefined;
  const topConsumers = data?.topConsumers as
    Array<{ vehicleId: string; licenceNumber: string; litres: string; amount: string }> | undefined;
  const reimbursements = data?.reimbursements as
    { totalPending: number; totalAmount: string } | undefined;
    ...
    <div className="space-y-6">
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <StatCard
          ...
          value={summary ? formatLitres(summary.totalLitres) : '—'}
        ...
        <StatCard
          ...
          value={summary ? formatCurrency(summary.avgCostPerLitre) : '—'}
          description="Weighted average"
          icon={<TrendingUp className="h-5 w-5" />}
        />
        <StatCard
          title="Pending Reimbursements"
          value={reimbursements ? String(reimbursements.totalPending) : '0'}
          description={reimbursements ? formatCurrency(reimbursements.totalAmount) : '—'}
                ...
                >
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="mt-1 flex gap-4">
                      <span className="text-ink-500 text-xs">{formatLitres(item.litres)}</span>
                      <span className="text-ink-500 text-xs">{formatCurrency(item.amount)}</span>
                    </div>
                  </div>
                  <div
                    aria-label={`Rank ${i + 1}`}
...
function ReportFleet({ data }: { data: Record<string, unknown> | null }) {
  ...
  if (!totalVehicles) {
    return (
      <Card>
        <CardContent>
          <p className="text-ink-400 py-8 text-center text-xs">No fleet data available.</p>
        </CardContent>
      </Card>
    );
  }
  ...
  const onTrip =
    distribution.find((d) => d.status === 'allocated' || d.status === 'issued')?.count ?? 0;
  ...
  const barItems = [
    {
      status: 'Available',
      count: available,
      pct: totalVehicles > 0 ? Math.round((available / totalVehicles) * 100) : 0,
      color: '#16a34a',
    },
    {
      status: 'On Trip',
      count: onTrip,
      pct: totalVehicles > 0 ? Math.round((onTrip / totalVehicles) * 100) : 0,
      color: '#2563eb',
    },
    {
      status: 'Maintenance',
      count: maintenance,
      pct: totalVehicles > 0 ? Math.round((maintenance / totalVehicles) * 100) : 0,
      color: '#ca8a04',
    },
    {
      status: 'Out of Service',
      count: outOfService,
      pct: totalVehicles > 0 ? Math.round((outOfService / totalVehicles) * 100) : 0,
      color: '#dc2626',
    },
  ].filter((b) => b.count > 0);
    ...
    <div className="space-y-6">
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <StatCard
          ...
          description="All active fleet"
          icon={<Truck className="h-5 w-5" />}
        />
        <StatCard
          title="Available"
          value={String(available)}
          description={`${totalVehicles > 0 ? Math.round((available / totalVehicles) * 100) : 0}% of fleet`}
          icon={<CheckCircle2 className="h-5 w-5" />}
          trend={{
            value:
              available > 0
                ? `${Math.round((available / totalVehicles) * 100)}% availability`
    ...
    <div className="space-y-6">
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <StatCard
          title="Completed"
          value={String(closed)}
          description={total > 0 ? `${Math.round((closed / total) * 100)}% completion` : '—'}
          icon={<CheckCircle2 className="h-5 w-5" />}
        />
        <StatCard
          title="Pending / Other"
          value={String(pending + other)}
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <StatCard
          ...
          value={
            routeCount > 0 ? `${Math.round(routeDistanceKm / routeCount).toLocaleString()} km` : '—'
          }
      ...
      <Card>
        ...
        <CardContent>
          {stats.length === 0 ? (
            <p className="text-ink-400 py-4 text-center text-xs">No trips for this period.</p>
          ) : (
            <div className="space-y-3">
              {stats.map((s) => (
                <div key={s.status} className="space-y-1">
                  <div className="flex items-center justify-between text-sm">
                    ...
                    <span className="text-ink-950 font-medium">
                      {s.totalTrips} ({total > 0 ? Math.round((s.totalTrips / total) * 100) : 0}%)
                    </span>
                  </div>
                  <div className="bg-muted h-2 w-full overflow-hidden rounded-full">
                    <div
                      className="bg-brand-600 h-full rounded-full transition-all"
                      style={{ width: `${total > 0 ? (s.totalTrips / total) * 100 : 0}%` }}
                    />
                  </div>
                </div>
              ))}
...
function ReportApprovals({ data }: { data: Record<string, unknown> | null }) {
  ...
  const stepsHistogram =
    (data?.stepsHistogram as Array<{ stepOrder: number; count: number }>) ?? [];
    ...
    <div className="space-y-6">
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <StatCard
          ...
          value={avgApprovalTime > 0 ? `${Math.round(avgApprovalTime)} hrs` : '—'}
        ...
        <StatCard
          title="Approval Rate"
          value={`${totalApprovalRate}%`}
          description={`${approved} approved, ${rejected} rejected, ${returned} returned`}
          icon={<TrendingUp className="h-5 w-5" />}
          trend={{
            value: totalApprovalRate >= 60 ? 'Above target (60%)' : 'Below target (60%)',
            positive: totalApprovalRate >= 60,
          }}
        />
      </div>
      ...
      {stepsHistogram.length > 0 && (
        <Card>
          <CardHeader>
            <CardTitle>Actions per Step</CardTitle>
          </CardHeader>
          <CardContent>
            <BarChart
      ...
      {totalActions === 0 && (
        <Card>
          <CardContent>
            <p className="text-ink-400 py-8 text-center text-xs">
              No approval actions recorded for this period. Workflow actions appear once approvals
              are processed.
            </p>
          </CardContent>
        </Card>
...
function ReportEnhanced({ data }: { data: Record<string, unknown> | null }) {
  ...
  const approvalTurnaround = enhanced?.approvalTurnaround as
    | {
        ...
        stepDurations: Array<{ stepOrder: number; avgHours: number }>;
        monthlyTrend: Array<{ month: string; avgHours: number; actionCount: number }>;
      }
    | undefined;
  const vehicleUtilisation = enhanced?.vehicleUtilisation as
    | {
        ...
        totalUtilisedHours: number;
        underUtilisedCount: number;
        underUtilisedVehicles: Array<{
  ...
  const fuelEfficiency = enhanced?.fuelEfficiency as
    | {
        fleetAvgKmPerLitre: number | null;
        totalLitres: number;
  ...
  const lateReturns = enhanced?.lateReturns as
    | {
        ...
        lateTrips: Array<{ vehicleLicence: string; actualHours: number; delayHours: number }>;
        monthlyLateTrend: Array<{ month: string; totalTrips: number; lateTrips: number }>;
      }
    | undefined;
  const rejectionMetrics = enhanced?.rejectionMetrics as
    | {
        ...
        approvalRate: number;
        rejectionReasons: Array<{ reason: string; date: string }>;
        monthlyTrend: Array<{ month: string; total: number; rejected: number; approved: number }>;
      }
    | undefined;
  ...
  if (!enhanced) {
    return (
      <Card>
        <CardContent>
          <p className="text-ink-400 py-8 text-center text-xs">
            No enhanced analytics data available. This report requires trip and approval data to be
            present.
          </p>
        </CardContent>
      </Card>
    ...
    <div className="space-y-6">
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <StatCard
          title="Avg. Approval Time"
          value={
            approvalTurnaround?.avgTotalHours != null
              ? `${approvalTurnaround.avgTotalHours} hrs`
              : '—'
          }
        ...
        <StatCard
          ...
          value={
            vehicleUtilisation?.avgUtilisation != null
              ? `${vehicleUtilisation.avgUtilisation}%`
              : '—'
          }
          ...
          trend={{
            value:
              (vehicleUtilisation?.avgUtilisation ?? 0) >= 50
                ? 'Good utilisation'
                : 'Low utilisation',
            positive: (vehicleUtilisation?.avgUtilisation ?? 0) >= 50,
          }}
        />
        <StatCard
        ...
        <StatCard
          ...
          icon={<AlertTriangle className="h-5 w-5" />}
          trend={{
            value: (lateReturns?.lateRate ?? 0) <= 10 ? 'Within tolerance' : 'High late rate',
            positive: (lateReturns?.lateRate ?? 0) <= 10,
          }}
        />
      </div>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              <Clock className="h-4 w-4" /> Approval Turnover Detail
            </CardTitle>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="grid grid-cols-2 gap-3">
              <div className="bg-muted rounded-[8px] p-3">
                <p className="text-ink-500 text-[11px] tracking-wider uppercase">Avg Hours</p>
                <p className="text-ink-950 text-xl font-bold">
                  {approvalTurnaround?.avgTotalHours ?? '—'}
                </p>
              </div>
              <div className="bg-muted rounded-[8px] p-3">
                <p className="text-ink-500 text-[11px] tracking-wider uppercase">Total Actions</p>
                <p className="text-ink-950 text-xl font-bold">
                  {approvalTurnaround?.totalActions ?? 0}
                </p>
              </div>
            </div>
            ...
            {approvalTurnaround?.stepDurations && approvalTurnaround.stepDurations.length > 0 && (
              <div>
                <p className="text-ink-700 mb-2 text-xs font-medium">Durations by Step</p>
                <div className="space-y-1.5">
                  {approvalTurnaround.stepDurations.map((s) => (
                    <div key={s.stepOrder} className="flex items-center justify-between text-sm">
                      ...
                      <span className="text-ink-950 font-medium">
                        {Math.round(s.avgHours * 10) / 10} hrs
                      </span>
                    </div>
                  ))}
                </div>
            ...
            {approvalTurnaround?.monthlyTrend && approvalTurnaround.monthlyTrend.length > 0 && (
              <div>
                <p className="text-ink-700 mb-2 text-xs font-medium">Monthly Trend</p>
                <BarChart
                  data={approvalTurnaround.monthlyTrend.map((m) => ({
                    label: m.month,
                    hours: Math.round(m.avgHours * 10) / 10,
                  }))}
        ...
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              <Truck className="h-4 w-4" /> Vehicle Utilisation
            </CardTitle>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="grid grid-cols-2 gap-3">
              <div className="bg-muted rounded-[8px] p-3">
                <p className="text-ink-500 text-[11px] tracking-wider uppercase">Fleet Avg</p>
                <p className="text-ink-950 text-xl font-bold">
                  {vehicleUtilisation?.avgUtilisation != null
                    ? `${vehicleUtilisation.avgUtilisation}%`
                    : '—'}
              ...
              <div className="bg-muted rounded-[8px] p-3">
                <p className="text-ink-500 text-[11px] tracking-wider uppercase">Under-Utilised</p>
                <p className="text-ink-950 text-xl font-bold">
                  {vehicleUtilisation?.underUtilisedCount ?? 0}
                </p>
              </div>
            </div>

            {vehicleUtilisation?.underUtilisedVehicles &&
              vehicleUtilisation.underUtilisedVehicles.length > 0 && (
                <div>
                  <p className="text-ink-700 mb-2 text-xs font-medium">Low-Utilisation Vehicles</p>
                  <div className="space-y-1.5">
                    {vehicleUtilisation.underUtilisedVehicles.slice(0, 5).map((v) => (
                      ...
                      >
                        ...
                        <span className="text-ink-950 font-medium">{v.utilisationPct}%</span>
                      </div>
                    ))}
                  </div>
                </div>
        ...
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              <Fuel className="h-4 w-4" /> Fuel Efficiency
            </CardTitle>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="bg-muted rounded-[8px] p-3">
                <p className="text-ink-500 text-[11px] tracking-wider uppercase">Total Distance</p>
                <p className="text-ink-950 text-xl font-bold">
                  {fuelEfficiency?.totalDistance != null
                    ? `${(fuelEfficiency.totalDistance / 1000).toFixed(1)}k km`
                    : '—'}
                </p>
              </div>
              <div className="bg-muted rounded-[8px] p-3">
                <p className="text-ink-500 text-[11px] tracking-wider uppercase">Route Distance</p>
                <p className="text-ink-950 text-xl font-bold">
                  {fuelEfficiency?.totalRouteKm != null
                    ? `${(fuelEfficiency.totalRouteKm / 1000).toFixed(1)}k km`
                    : '—'}
                </p>
              </div>
              <div className="bg-muted rounded-[8px] p-3">
                <p className="text-ink-500 text-[11px] tracking-wider uppercase">Total Cost</p>
                <p className="text-ink-950 text-xl font-bold">
                  {fuelEfficiency?.totalFuelCost != null
                    ? formatCurrency(fuelEfficiency.totalFuelCost)
                    : '—'}
                </p>
              </div>
            </div>
            ...
            {fuelEfficiency?.perVehicle && fuelEfficiency.perVehicle.length > 0 && (
              <>
                <BarChart
                  data={fuelEfficiency.perVehicle
                    .slice(0, 8)
                    .map((v) => ({ label: v.licenceNumber, kmpl: v.kmPerLitre ?? 0 }))}
                  bars={[{ key: 'kmpl', color: '#16a34a', label: 'km/L' }]}
                  height={120}
                />
                <div>
                  <p className="text-ink-700 mb-2 text-xs font-medium">Driven vs Planned (km)</p>
                  <BarChart
                  ...
                  <table className="w-full text-sm">
                    ...
                    <tbody>
                      {fuelEfficiency.perVehicle.slice(0, 8).map((v) => (
                        ...
                        >
                          <td className="text-ink-950 px-2 py-2 font-medium">{v.licenceNumber}</td>
                          <td className="text-ink-700 px-2 py-2 text-right tabular-nums">
                            {v.routeDistanceKm ? v.routeDistanceKm.toLocaleString() : '—'}
                          </td>
                          <td className="text-ink-700 px-2 py-2 text-right tabular-nums">
                            {v.estimatedDistanceKm ? v.estimatedDistanceKm.toLocaleString() : '—'}
                          </td>
                          <td className="text-ink-700 px-2 py-2 text-right tabular-nums">
                            {v.totalLitres.toLocaleString()}
                          </td>
                          <td className="text-ink-700 px-2 py-2 text-right tabular-nums">
                            {v.kmPerLitre != null ? v.kmPerLitre : '—'}
                          </td>
                        </tr>
                      ))}
                    </tbody>
            ...
            {fuelEfficiency?.routeKmTrend && fuelEfficiency.routeKmTrend.length > 0 && (
              <div>
                <p className="text-ink-700 mb-2 text-xs font-medium">Route Distance Trend</p>
                <BarChart
                  data={fuelEfficiency.routeKmTrend.map((m) => ({
                    label: m.month,
                    km: Math.round(m.routeKm),
                  }))}
        ...
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              <Clock className="h-4 w-4" /> Late Returns
            </CardTitle>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="grid grid-cols-3 gap-3">
              <div className="bg-muted rounded-[8px] p-3">
                <p className="text-ink-500 text-[11px] tracking-wider uppercase">Late Rate</p>
                <p className="text-ink-950 text-xl font-bold">{lateReturns?.lateRate ?? 0}%</p>
              </div>
              <div className="bg-muted rounded-[8px] p-3">
                <p className="text-ink-500 text-[11px] tracking-wider uppercase">Late Count</p>
                <p className="text-ink-950 text-xl font-bold">{lateReturns?.lateCount ?? 0}</p>
              </div>
              <div className="bg-muted rounded-[8px] p-3">
                <p className="text-ink-500 text-[11px] tracking-wider uppercase">Avg Delay</p>
                <p className="text-ink-950 text-xl font-bold">
                  {lateReturns?.avgDelayHours ?? 0} hrs
                </p>
              </div>
            </div>
            ...
            {lateReturns?.lateTrips && lateReturns.lateTrips.length > 0 && (
              <div>
                <p className="text-ink-700 mb-2 text-xs font-medium">Recent Late Trips</p>
                <div className="space-y-1.5">
                  {lateReturns.lateTrips.slice(0, 5).map((t, i) => (
                    <div key={i} className="flex items-center justify-between text-sm">
                      <span className="text-ink-600">{t.vehicleLicence}</span>
                      <span className="text-ink-950 font-medium">
                        {Math.round(t.delayHours * 10) / 10} hrs late
                      </span>
                    </div>
                  ))}
                </div>
        ...
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              <XCircle className="h-4 w-4" /> Rejection Metrics
            </CardTitle>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="grid grid-cols-3 gap-3">
              <div className="bg-muted rounded-[8px] p-3">
                <p className="text-ink-500 text-[11px] tracking-wider uppercase">Rejection Rate</p>
                <p className="text-ink-950 text-xl font-bold">
                  {rejectionMetrics?.rejectionRate ?? 0}%
                </p>
              </div>
              <div className="bg-muted rounded-[8px] p-3">
                <p className="text-ink-500 text-[11px] tracking-wider uppercase">Approval Rate</p>
                <p className="text-ink-950 text-xl font-bold">
                  {rejectionMetrics?.approvalRate ?? 0}%
                </p>
              </div>
              <div className="bg-muted rounded-[8px] p-3">
                <p className="text-ink-500 text-[11px] tracking-wider uppercase">Rejected</p>
                <p className="text-ink-950 text-xl font-bold">{rejectionMetrics?.rejected ?? 0}</p>
              </div>
            </div>

            {rejectionMetrics?.rejectionReasons && rejectionMetrics.rejectionReasons.length > 0 && (
              <div>
                <p className="text-ink-700 mb-2 text-xs font-medium">Recent Rejection Reasons</p>
                <div className="space-y-1.5">
                  {rejectionMetrics.rejectionReasons.slice(0, 5).map((r, i) => (
                    <div key={i} className="flex items-start justify-between gap-2 text-sm">
                      <span className="text-ink-600 line-clamp-1 flex-1">{r.reason}</span>
                      <span className="text-ink-400 shrink-0 text-[11px]">{r.date}</span>
                    </div>
                  ))}
                </div>
              </div>
            ...
            {rejectionMetrics?.monthlyTrend && rejectionMetrics.monthlyTrend.length > 0 && (
              <div>
                <p className="text-ink-700 mb-2 text-xs font-medium">Monthly Trend</p>
                <BarChart
                  data={rejectionMetrics.monthlyTrend.map((m) => ({
                    label: m.month,
                    Total: m.total,
                    Rejected: m.rejected,
                    Approved: m.approved,
        ...
        {lateReturns?.monthlyLateTrend && lateReturns.monthlyLateTrend.length > 0 && (
          <Card>
            <CardHeader>
              <CardTitle className="flex items-center gap-2">
                <BarChart3 className="h-4 w-4" /> Late Return Trend (Monthly)
              </CardTitle>
            </CardHeader>
            <CardContent>
              <BarChart
                data={lateReturns.monthlyLateTrend.map((m) => ({
                  label: m.month,
                  Total: m.totalTrips,
                  Late: m.lateTrips,
                }))}
        ...
        {approvalTurnaround?.monthlyTrend && approvalTurnaround.monthlyTrend.length > 0 && (
          <Card>
            <CardHeader>
              <CardTitle className="flex items-center gap-2">
                <TrendingUp className="h-4 w-4" /> Approval Duration Trend
              </CardTitle>
            </CardHeader>
            <CardContent>
              <BarChart
...
function ReportRequests({ data }: { data: Record<string, unknown> | null }) {
  ...
  const pending =
    stats.find((s) => s.status === 'submitted' || s.status === 'pending')?.totalRequests ?? 0;
    ...
    <div className="space-y-6">
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <StatCard
          ...
          description={total > 0 ? `${Math.round((approved / total) * 100)}% approval rate` : '—'}
          icon={<CheckCircle2 className="h-5 w-5" />}
        />
        <StatCard
          title="Pending"
        ...
        <StatCard
          ...
          description={total > 0 ? `${Math.round((rejected / total) * 100)}% rejection rate` : '—'}
      ...
      <Card>
        ...
        <CardContent>
          {stats.length === 0 ? (
            <p className="text-ink-400 py-4 text-center text-xs">No requests for this period.</p>
          ) : (
            <div className="space-y-3">
              {stats.map((s) => (
                <div key={s.status} className="space-y-1">
                  <div className="flex items-center justify-between text-sm">
                    ...
                    <span className="text-ink-950 font-medium">
                      {s.totalRequests} (
                      {total > 0 ? Math.round((s.totalRequests / total) * 100) : 0}%)
                    </span>
                  </div>
                  <div className="bg-muted h-2 w-full overflow-hidden rounded-full">
                    <div
                      className="bg-brand-600 h-full rounded-full transition-all"
                      style={{ width: `${total > 0 ? (s.totalRequests / total) * 100 : 0}%` }}
                    />
                  </div>
                </div>
              ))}
