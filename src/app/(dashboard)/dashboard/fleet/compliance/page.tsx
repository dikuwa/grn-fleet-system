        ...
        <>
          ...
          <div className="grid gap-4 sm:grid-cols-4">
            ...
            <Card>
              <CardContent className="pt-4">
                <div className="text-center">
                  <p className="text-ink-500 text-2xl font-[650] tabular-nums">
                    {summary.incomplete}
                  </p>
                  <p className="text-ink-500 text-xs">Incomplete</p>
                </div>
              </CardContent>
            </Card>
          </div>
          ...
          {upcoming.filter((u) => u.status === 'expired' || u.status === 'expiring_soon').length >
            0 && (
            <Card className="border-amber-200 bg-amber-50/50 dark:border-amber-800 dark:bg-amber-950/30">
              ...
              <CardContent>
                <div className="space-y-2">
                  {upcoming
                    .filter((u) => u.status === 'expired' || u.status === 'expiring_soon')
                    .slice(0, 10)
                    .map((u) => (
                      <div
                        key={`${u.vehicleId}-${u.type}`}
                        className="bg-surface flex items-center justify-between rounded-lg px-4 py-2 text-sm"
                      >
                        <div className="flex items-center gap-3">
                          <div
                            className={`h-2 w-2 rounded-full ${u.status === 'expired' ? 'bg-red-500' : 'bg-amber-500'}`}
                          />
                          <span className="text-ink-950 font-medium">{u.licenceNumber}</span>
          ...
          <div className="flex flex-wrap items-center gap-4">
            <div className="relative w-full flex-1 sm:max-w-xs">
              ...
              <input
                ...
                className="border-border bg-surface h-10 w-full rounded-[8px] border pr-3 pl-9 text-sm"
              />
            </div>
            <StyledSelect
              value={filterStatus}
          ...
          <div className="space-y-3">
            {filtered.length === 0 ? (
              ...
              filtered.map((v) => (
                <Link
                  key={v.vehicleId}
                  href={`/dashboard/fleet/${v.vehicleId}`}
                    ...
                    <div className="flex min-w-0 items-center gap-4">
                      ...
                      <div className="min-w-0">
                        <div className="flex items-center gap-2">
                          ...
                          <Badge variant={overallVariant(v.overallStatus)} size="sm">
                            {overallLabel(v.overallStatus)}
                          </Badge>
                        </div>
                        <p className="text-ink-500 mt-0.5 text-xs">{v.licenceNumber}</p>
                        <div className="mt-1.5 flex items-center gap-3">
                          {v.items.map((item) => (
                            <span
                              key={item.type}
                              className="flex items-center gap-1 text-xs"
                              title={daysLabel(item.daysRemaining)}
                            >
                              <span
