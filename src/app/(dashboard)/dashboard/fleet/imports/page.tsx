...
import { Upload, Database, FileSpreadsheet, ChevronRight, Truck } from 'lucide-react';
import Link from 'next/link';

export const dynamic = 'force-dynamic';

async function fetchVehicleImportHistory() {
  const dbo = getDb();
  return dbo
    .select()
    .from(importBatches)
    .where(eq(importBatches.importType, 'vehicles'))
    .orderBy(desc(importBatches.createdAt))
    .limit(50);
}

export default async function VehicleImportHistoryPage() {
  if (!isDbConnected()) {
    return (
      <div className="space-y-6">
        <Breadcrumbs items={[{ label: 'Dashboard', href: '/dashboard' }, { label: 'Fleet', href: '/dashboard/fleet' }, { label: 'Import History' }]} />
        <PageHeader title="Vehicle Import History" description="Previous vehicle data imports" />
  ...
  let batches: Awaited<ReturnType<typeof fetchVehicleImportHistory>>;
  try {
    batches = await fetchVehicleImportHistory();
  } catch (error) {
    console.error('Vehicle import history query failed:', error);
    return (
      <div className="space-y-6">
        <Breadcrumbs items={[{ label: 'Dashboard', href: '/dashboard' }, { label: 'Fleet', href: '/dashboard/fleet' }, { label: 'Import History' }]} />
        <PageHeader title="Vehicle Import History" description="Previous vehicle data imports" />
  ...
  return (
    <div className="space-y-6">
      <Breadcrumbs items={[{ label: 'Dashboard', href: '/dashboard' }, { label: 'Fleet', href: '/dashboard/fleet' }, { label: 'Import History' }]} />
      <PageHeader
        title="Vehicle Import History"
        description={batches.length > 0 ? `${batches.length} previous import${batches.length !== 1 ? 's' : ''}` : 'Track vehicle data imports'}
      >
        ...
        <Card>
          <CardContent className="py-12">
            <EmptyState
              ...
              description="Vehicle data imports will appear here once you upload and process a CSV file."
              action={{ label: 'Import Vehicles', onClick: () => { window.location.href = '/dashboard/fleet/import'; } }}
            />
          </CardContent>
        </Card>
      ) : (
