...
function OdometerTimeline({ vehicleId }: { vehicleId: string }) {
    ...
    void fetch(`/api/fleet/${vehicleId}/odometer`, { signal: controller.signal, cache: 'no-store' })
      ...
      .catch((reason) => {
        if (!controller.signal.aborted)
          setError(reason instanceof Error ? reason.message : 'Could not load odometer history');
      })
      .finally(() => {
  ...
  if (loading) {
    return (
      <div className="text-ink-500 flex items-center justify-center gap-2 px-5 py-12 text-sm">
  ...
  if (error)
    return (
      <div className="text-status-error-text px-5 py-8 text-sm" role="alert">
        {error}
      </div>
    );
  ...
  return (
    <div>
      <div className="border-border bg-muted/30 border-b px-5 py-4">
        ...
        <p className="text-ink-950 mt-1 text-2xl font-semibold tabular-nums">
          {(currentOdometer ?? 0).toLocaleString()} km
        </p>
      </div>
      {events.length === 0 ? (
        <p className="text-ink-500 px-5 py-8 text-sm">
          No immutable odometer events have been recorded for this vehicle yet.
        </p>
      ) : (
            ...
            return (
              <div key={event.id} className="px-5 py-3">
                <div className="flex flex-wrap items-start justify-between gap-3">
                  <div>
                    <p className="text-ink-950 text-sm font-semibold tabular-nums">
                    ...
                    <p className="text-ink-500 mt-1 text-xs capitalize">
                      ...
                      {event.sourceEntityType
                        ? ` · ${event.sourceEntityType.replaceAll('_', ' ')}`
                        : ''}
                    </p>
                  ...
                  <div className="text-right">
                    ...
                    {delta !== null && delta >= 0 && (
                      <p className="text-status-info-text mt-1 text-xs font-medium tabular-nums">
...
export function TabsShell({ children, visibleTabs = DEFAULT_VISIBLE_TABS }: TabsShellProps) {
  ...
  const tabs = useMemo(() => TABS.filter((tab) => visibleTabs.includes(tab.key)), [visibleTabs]);
  ...
  const content =
    activeTab === 'odometer' ? <OdometerTimeline vehicleId={vehicleId} /> : contentByKey[activeTab];
  ...
  return (
    <Card>
      <PageTabs
        ...
        label="Vehicle record sections"
      />
      <div role="tabpanel">
        {content ?? (
          <div className="text-ink-500 px-5 py-8 text-center text-sm">No content available.</div>
        )}
      </div>
    </Card>
  );
