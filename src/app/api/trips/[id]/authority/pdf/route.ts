...
export async function GET(
  ...
  try {
    ...
    const [passengers, drivers, progress, incidents, defects] = await Promise.all([
      db.select().from(tripAuthorityPassengers).where(eq(tripAuthorityPassengers.authorityId, authority.authority.id)),
      db.select({
      ...
      db.select().from(tripProgressEntries).where(and(eq(tripProgressEntries.tripId, id), eq(tripProgressEntries.tenantId, session.tenantId))),
      db.select().from(tripIncidents).where(and(eq(tripIncidents.tripId, id), eq(tripIncidents.tenantId, session.tenantId))),
    ...
    const token = (authority.authority.data as { verificationToken?: string } | null)?.verificationToken;
    const baseUrl = process.env.NEXT_PUBLIC_APP_URL || 'http://localhost:3000';
    const verificationUrl = token ? `${baseUrl}/verify/authority/${encodeURIComponent(token)}` : undefined;
    const qrCodeDataUrl = verificationUrl ? await QRCode.toDataURL(verificationUrl, { width: 220, margin: 1 }) : undefined;
    const distance = authority.authority.beginningOdometer !== null && authority.authority.endingOdometer !== null
    ...
    const data: TripAuthorityData = {
      ...
      additionalDrivers: drivers.filter((driver) => driver.driverType !== 'primary').map((driver) => ({
    ...
    return NextResponse.json({ error: 'Official Trip Authority PDF could not be generated' }, { status: 500 });
  }
}
