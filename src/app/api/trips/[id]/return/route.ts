...
export async function POST(
  ...
  try {
    ...
    if (!employee || employee.id !== trip.driverEmployeeId) {
      return NextResponse.json({ error: 'Only the assigned driver may return this trip' }, { status: 403 });
    }
    if (!['in_progress', 'return_due'].includes(trip.trip.status)) {
      return NextResponse.json({ error: `Cannot return trip with status "${trip.trip.status}". Only in-progress or return-due trips can be returned.` }, { status: 409 });
    }
    ...
    await runAtomicMutations((tx) => [
      tx.update(trips)
        .set({ status: 'return_inspection', returnedAt: now, updatedAt: now })
        .where(and(eq(trips.id, id), eq(trips.tenantId, session.tenantId))),
      tx.update(tripAuthorities)
        .set({
          status: 'awaiting_arrival_inspection',
          endingOdometer: Number(body.endingOdometer),
        ...
        .where(and(
          ...
          inArray(tripAuthorities.status, ['in_progress', 'delayed', 'route_deviation_pending_review', 'incident_reported']),
        )),
      tx.insert(vehicleOdometerEvents).values({
      ...
      tx.update(vehicles)
        .set({ currentOdometer: Number(body.endingOdometer), updatedAt: now })
    ...
    try {
      ...
      console.warn('[trips/return] Post-commit activity failed:', err);
    }
