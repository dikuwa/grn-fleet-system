...
async function requireActiveTransportReviewer(
  request: NextRequest,
  id: string,
) {
  ...
  return { session, context } as const;
}
...
export async function POST(
  ...
  if (reason.length > 500) {
    return NextResponse.json({ error: 'Correction reason must be 500 characters or fewer.' }, { status: 422 });
  }
  ...
  if (purpose.length > 2000) {
    return NextResponse.json({ error: 'Purpose must be 2,000 characters or fewer.' }, { status: 422 });
  }
        ...
        tx
          ...
          .where(
            and(
              ...
              eq(transportRequests.status, 'transport_review'),
              eq(transportRequests.version, context.request.version),
              eq(transportRequests.workflowInstanceId, context.request.workflowInstanceId),
            ),
          ),
