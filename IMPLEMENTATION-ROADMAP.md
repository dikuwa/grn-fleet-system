...
| 2.7  | Users                        | VERIFIED    | Admin user list/detail, invite dialog, PATCH API for role/status updates, full access lifecycle (remove with active-role gate, soft-remove preserves staff, restore endpoint, Removed Access tab) | ✅ Verified with `user-access-lifecycle` integration tests       | HIGH     |
...
| 3.2  | Approval workflow      | VERIFIED    | WorkflowEngine class, regional/national definitions, action processing, separation of duty, emergency override, 10 E2E tests, Transport Decision workspace (assign/replace vehicle + driver with live eligibility verdicts, driver-replacement notifications) wired into the approval action page at the transport_review step | ✅ Full approval → trip workflow tested                | HIGHEST  |
...
| 3.4  | Vehicle recommendation | VERIFIED    | VehicleRecommender class (deterministic scoring), category/terrain/passenger matching, `recommendOnly` advisory mode in allocation POST (no side effects) | ✅ Wired into allocation UI with scores/reasons        | HIGH     |
| 3.5  | Vehicle allocation     | VERIFIED    | New-allocation flow rewritten: searchable eligible-request selector (`GET /api/allocations/requests`), vehicle picker (no UUID entry, no auto-create-on-recommend), driver selector with real-time compliance verdicts (`GET /api/allocations/drivers`), server-side double-booking + driver-compliance validation, driver/requester notifications, request → vehicle_allocated transition | ✅ End-to-end allocation from approved request         | HIGH     |
| 3.6  | Driver assignment      | VERIFIED    | Driver picker at allocation creation (server-validated), `DriverAssignment` component at allocation detail, assign/unassign API, driver detail page with licence/assignment history | ✅ Driver eligibility enforced on frontend + backend    | HIGH     |
...
| 4.2 | Driver management   | VERIFIED    | Driver list (licence search, status filter, pagination, server-side stats), licence-expiry alert banner + Expiring Soon filter (click-through to the verification queue), detail page with licences (expiry status), assignment history, driver mobile view, self-service portal | ✅ Complete driver management                    | MEDIUM   |
| 4.8 | Licence verification | VERIFIED    | Dedicated queue (`/dashboard/drivers/licences`, 7 tabs: pending/expiring/expired/changes-requested/rejected/approved/all) + review screen (`/dashboard/drivers/licences/[id]`): zoomable front/back images, OCR provisional vs verified values, warnings (number/class mismatch, future issue, passed expiry), corrections, version history, approve (supersedes previous + recalculates eligibility) / request changes / reject with reason + driver notifications; queue API with search/filter/pagination/server stats; readiness gate uses active-verified licence (not newest-expiry) | ✅ Renewal workflow versioned end-to-end, expiry-digest recipients notified on upload | MEDIUM   |
| 4.3 | Fuel management     | VERIFIED    | Fuel list, new entry with offline draft support, fuel API, reimbursement API, on-behalf-of driver attribution (`driverEmployeeId` via `fuel_transactions.driver_employee_id`, validated against trip/tenant, driver picker on the form, attribution shown on list + detail) | ✅ Works end-to-end, driver attribution persisted in production (migration 0032) | HIGH     |
...
| 4.6 | Expiry alerts       | VERIFIED    | Expiry alerts dashboard at `/dashboard/expiry-alerts`, Inngest crons for licence/roadworthy/insurance expiry, digest core extracted + integration-tested | ✅ Driver + vehicle licence expiry alerts verified end-to-end | MEDIUM   |
...
| 5.7 | Background jobs        | VERIFIED    | Inngest configured, 6 functions: step reminder, escalation, approval completed, vehicle+driver licence expiry crons, maintenance reminder | ✅ All tenant-isolated, idempotent digest verified via integration test | MEDIUM   |
...
| #   | Module                     | Status      | What Exists                                                                                                              | What's Missing                                                 | Priority |
| --- | -------------------------- | ----------- | ------------------------------------------------------------------------------------------------------------------------ | -------------------------------------------------------------- | -------- |
| 7.1 | Driver mobile workflow     | VERIFIED    | Driver mobile view (`/dashboard/driver-mobile`), self-service portal (`/dashboard/driver-self-service`), driver-mobile PWA E2E spec (dashboard shows assigned trip at 375×812 viewport) | ✅ Both mobile-optimized views built + E2E tested             | MEDIUM   |
| 7.2 | Offline inspection drafts  | VERIFIED    | Dexie stores (fuel, transport requests, inspections), offline-sync handler, offline-status component with draft count, departure/return pages use canonical template checklists | ✅ Auto-sync on reconnect + mount-time sync + 60s polling     | MEDIUM   |
...
| 7.4 | Sync and conflict handling | VERIFIED    | Offline sync service with pending/synced/failed/conflict states, conflict resolution UI at `/dashboard/offline`, device-owned draft scope fix, idempotent inspection sync | ✅ Full conflict resolution UI + E2E save→sync→idempotency    | MEDIUM   |
...
| #   | Module                        | Status      | What Exists                                                                                                                                                                                                                                                         | What's Missing                                                                    | Priority |
| --- | ----------------------------- | ----------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------- | -------- |
| 8.1 | End-to-end testing            | VERIFIED    | 10 regional trip workflow E2E tests (Playwright), offline drafts E2E tests, driver-mobile offline PWA E2E (2), 28 integration tests (auth + documents)                                                                                                                 | Full workflow covered                                                             | MEDIUM   |
| 8.2 | Cross-tenant security testing | VERIFIED    | 13 Vitest test cases covering all core entities + browser-level Playwright suite (`src/e2e/cross-tenant-isolation.spec.ts`, 5 tests: fleet/reports, vehicle by-id, requests, audit/notifications, platform-admin boundary)                                                | ✅ All 13 Vitest + 5 Playwright pass                                                  | HIGH     |
...
1. **Driver mobile PWA E2E spec** (`src/e2e/driver-mobile-offline.spec.ts`) — 2 tests at 375×812 viewport: driver-mobile dashboard lists the assigned trip; offline departure inspection saves a draft, reconnects, auto-syncs to a DB row and is idempotent on re-sync. Self-cleaning `beforeAll` mirrors seed-e2e (cancels stale allocations/trips) so repeated runs never 409 on driver/vehicle overlap; odometer derived from the chosen vehicle's DB row; poll window widened so the 60s retry interval is always caught. ✅
2. **Cross-tenant browser security suite** (`src/e2e/cross-tenant-isolation.spec.ts`) — 5 Playwright tests using the same login helper as the other specs: tenant A fleet listing + reports never leak tenant B vehicles; tenant A cannot read or mutate tenant B vehicle by id; tenant A cannot see or cancel a tenant B transport request; tenant A audit log + notifications never include tenant B events; tenant users cannot reach platform administration while the platform admin can. ✅
3. **Inspection checklist bug (product bug the spec exposed)** — `departure`/`return` pages shipped hardcoded 28-item checklists whose labels did NOT match the active server template (`DEPARTURE_INSPECTION_ITEMS` / `RETURN_INSPECTION_ITEMS`, 16/9 items). The API validates exactly-once by label, so every submission (online or offline-sync) returned 422. Both pages now render the canonical template items with the dynamic photo gate. ✅
4. **DRIVER inspection access (permission-model fix)** — DRIVER role grants `INSPECTION_PERFORM`, but the inspection page/API workspaces (`dashboard-access.ts`) and the DRIVER policy (`permissions.ts`) lacked the right to open/perform inspections. Added DRIVER to the `inspections-departure`/`return`/`new` page workspaces with create access and granted `INSPECTION_PERFORM` in the DRIVER policy, matching the role definition. ✅
5. **Mount-time offline sync (product gap)** — `OfflineSyncHandler` only synced on the `online` event or the 60s interval, so a draft saved mid-session could sit unsynced indefinitely (and Playwright's `setOffline(false)` never fires the event). The handler now syncs pending drafts immediately on mount once the profile resolves. ✅
6. **Orphaned-draft scope fix (product bug)** — a draft saved before the profile query resolved carries `userId/tenantId: null`; `listDrafts`/`countUnsyncedDrafts` filtered by exact owner so the unowned draft was excluded from sync forever. Null-scoped drafts are now treated as device-owned and remain eligible for sync by whoever is logged in (single-user PWA device model). ✅
7. **Generic branded email fallback** (`src/lib/email.ts` + `src/emails/notification.tsx`) — registry gains a `notification` template entry plus `action_required`/`awareness`/`outcome`/`alert` category aliases so the workflow engine and notifications route never fall through to a missing template (fixes unhandled `document_expiry_` prefix mapping too). ✅
8. **Validation** — `pnpm tsc --noEmit` 0 errors; `pnpm lint` 0 errors (2 pre-existing warnings in untouched lines of `functions.ts`); `pnpm vitest run` 265/265 passed (23 files); Playwright driver-mobile (2/2) + cross-tenant (5/5) passed against a fresh production build + e2e seed; `pnpm build` green. ✅

### Session 36 ✅ — User Access Lifecycle, Phase 32 Driver Inspection Scope, Offline Incident E2E, Cross-Tenant Integration Tests

1. **User access lifecycle (DELETE → soft remove, staff preserved)** (`src/app/api/admin/users/[id]/route.ts`) — Rewrote DELETE with an ACTIVE-ROLE GATE (role-count rule — only currently in-force assignments block; historical/ended assignments never block), final Tenant Administrator protection (the last active TA — including self — cannot be removed), dependency checks (open trips or live allocations as assigned driver must be reassigned first), session revocation + verification-token invalidation before the state change, and an atomic soft-remove: membership → `access_removed` + user profile → `removed` inside a `db.transaction()` while the linked staff/employee record is preserved (the person still appears in Staff Directory). Audit event carries full context (email, activeRoleCount, sessionsRevoked, verificationTokensInvalidated). ✅
2. **Role removal = soft close with history** — PATCH no longer deletes role assignments: it writes an `endDate` (history preserved for audit/workflow reconstruction), allows re-assigning the same role after the end date (active-assignment check only), blocks re-activation of `access_removed` accounts via generic status PATCH (must use Restore), protects the final Tenant Administrator on role removal, and writes an `role_assignment.ended` audit event. ✅
3. **Removed-account visibility** — GET `/api/admin/users` hides `access_removed` accounts from the default view and surfaces them via the explicit `?status=removed` filter (plus a new `?status=pending` filter). User Management UI gained a "Removed Access" tab with per-row Restore buttons + confirm dialog, and the detail page shows a status badge, a danger-zone "Remove User Access" with explainer, and a "Restore User Access" action for removed accounts. ✅
4. **Restore endpoint** (`src/app/api/admin/users/[id]/restore/route.ts`) — POST re-activates `access_removed` → `active` (+ profile) in a transaction, `TENANT_MANAGE` gated, cross-tenant safe, `user_access.restored` audit event. ✅
5. **Phase 32 driver inspection scope** — Removed DRIVER from the `/dashboard/inspections/new|departure|return` page workspaces (`dashboard-access.ts`) and dropped `INSPECTION_PERFORM` from the DRIVER workspace policy + DRIVER RoleDefinition (`permissions.ts`). `DriverTripWorkspace` no longer offers Start Inspection / Arrival Inspection buttons — official inspections are performed by Inspectors / Release Officers; drivers report incidents, defects and damage from the trip console (retaining `FILE_UPLOAD` for photos). ✅
6. **E2E rework** (`src/e2e/driver-mobile-offline.spec.ts`) — Phase-32 compliant: the offline flow now tests **incident reporting** instead of a driver-performed departure inspection. Full arc: inspector departure inspection (canonical 16-item checklist) → issue → driver acknowledge → start → offline incident report saved as a Dexie draft → reconnect → mount-time auto-sync → DB row with `idempotentReplay: true` on re-sync. Self-cleaning `beforeAll` mirrors seed-e2e. ✅
...
8. **Validation** — `pnpm tsc --noEmit` 0 errors (fixed during review: `session` table-import shadowing in DELETE — now `sessionTable`; widened `activeTab` union to include `removed`; driver-column join predicate for the open-trip dependency check); `pnpm lint` 0 errors (2 pre-existing warnings in untouched lines of `functions.ts`); `pnpm vitest run` 265/265 passed (23 files).
...
### Session 37 ✅ — Expiry-Digest Core Extraction + Verification, Integration Suite Repaired (username collision, neon transactions, cross-test race)

1. **Digest core extracted** (`src/lib/inngest/expiry-digest.ts`) — The `driverLicenceExpiryDigest` cron body moved out of `functions.ts` into a testable `runDriverLicenceExpiryDigest({ tenantIds, now, skipEmails })` module (same tenant-scoped, idempotent day-epoch behaviour; now includes already-expired licences per spec). The cron is now a thin wrapper. ✅
2. **Integration test** (`src/test/expiry-digest.integration.test.ts`) — 3/3 passing against the seeded server: digest created for a business day (recipients resolved from active Transport Administrators, body lists the expiring licence, `eventVersion` = day-epoch), idempotent on re-run (no duplicate notification), already-expired licences included, and nothing sent to tenants without expiring licences/admins (cross-tenant leak negative). ✅
3. **Username collision fix** (`src/app/api/admin/users/route.ts`) — POST derived `username` from the display name, so two accounts sharing a name (or re-creating after a soft-remove, which keeps the `user` row) 500'd on `user_username_key`. Username is now derived from the unique email local part when not supplied — no more 500s. ✅
4. **Neon transaction fix** (`src/app/api/admin/users/[id]/route.ts` + `restore/route.ts`) — `db.transaction()` is unsupported by the neon-http driver (CI ran SQLite, so Session 36's DELETE/RESTORE never hit it). Both now run the two idempotent updates sequentially. This was the last real failure in the access-lifecycle integration suite. ✅
5. **Integration suite repaired** — Fixed pre-existing failures exposed by running the full suite against Neon: `auth.integration.test.ts` asserted lowercase `'active'` tenant status (seed stores `'ACTIVE'`); new permission codes (`TRIP_AUTHORITY_OVERRIDE_NUMBER`, `USER_VIEW`, `USER_MANAGE_STATUS`, `USER_INVITE`) were missing from `PermissionGroups` in `src/lib/permissions.ts` (permissions suite + matrix gap); `user-access-lifecycle.integration.test.ts` had 9 response-body-consumed assertions (`expect(res.status, await res.text())` then `.json()` → “body already read”), wrong vitest timeout argument order, and depended on seeded `availableEmployees[0]` — which the parallel expiry-digest suite deletes mid-run (cross-test race). It now creates its own employee fixture and cleans up in `finally`. ✅
6. **Lint cleanup** (`src/lib/inngest/functions.ts`) — removed the 2 pre-existing unused-variable warnings (`db` in `approvalCompleted`, `notifications` import in `tripReturnDueCheck`). Suite is now 0 errors / 0 warnings. ✅
7. **Validation** — `pnpm tsc --noEmit` 0 errors; `pnpm lint` 0 errors 0 warnings; `pnpm vitest run` 265/265 (23 files); `pnpm test:integration` **47/47 (5 files)** including the new digest spec (3/3) and access-lifecycle (2/2). Dev server restarted on :3000 with the fixes live; preview re-registered. ✅
